# v0.1.129 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- reversed card ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.128 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- reversed card ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.127 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- famous fox icon ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.126 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- famous fox button and icon sizing ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.125 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- famous fox btn ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.124 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- remove text hover on social buttons ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.123 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- spinner component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.122 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- spinner component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.121 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- attempting to resolve background location for modal layout from nav link ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.120 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- trying to resolve overlooked tw properties ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.119 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- resolving circular dep in modal component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.118 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- forgot to export context dir ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.117 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- modal context and layout ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.116 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- nav logo sizing ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.115 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- export of main layout ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.114 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- debugging export of main layout ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.113 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- main layout and nav links ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.112 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- scroll to top on click for magnum logo link to home ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.111 (Fri Jun 17 2022)

#### ⚠️ Pushed to `master`

- scroll margin on sections ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.110 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- reverting card styles ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.109 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- gradient dir ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.108 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- gradient dir ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.107 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- missing comma ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.106 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- github ([@yondav](https://github.com/yondav))
- card gradient dirextion ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.105 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- overlay z index on nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.104 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- hamburger padding ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.103 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- section padding ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.102 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- html min width ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.101 (Wed Jun 15 2022)

#### ⚠️ Pushed to `master`

- larger hamburger in nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.100 (Tue Jun 14 2022)

#### ⚠️ Pushed to `master`

- smooth scroll ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.99 (Wed Jun 08 2022)

#### ⚠️ Pushed to `master`

- reverted image container ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.98 (Wed Jun 08 2022)

#### ⚠️ Pushed to `master`

- gradient borders ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.97 (Wed Jun 08 2022)

#### ⚠️ Pushed to `master`

- next and back icons ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.96 (Wed Jun 08 2022)

#### ⚠️ Pushed to `master`

- card padding for mobile ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.95 (Wed Jun 08 2022)

#### ⚠️ Pushed to `master`

- button redesign ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.94 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- modal and card tweaking ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.93 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- react router index link on nav bar ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.92 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- syntax correction ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.91 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- featured image wrapper ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.90 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- featured img wrapper ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.89 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- card image props ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.88 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- image card padding ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.87 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- transitions on card elements ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.86 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- exporting gradient divider ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.85 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- image container component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.84 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- section padding ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.83 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- section padding ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.82 (Tue Jun 07 2022)

#### ⚠️ Pushed to `master`

- general styles ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.81 (Mon Jun 06 2022)

#### ⚠️ Pushed to `master`

- header styles, card spacing and gradient lines, card header variation ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.80 (Mon Jun 06 2022)

#### ⚠️ Pushed to `master`

- indicator bullets for card component to be used on roadmap ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.79 (Mon Jun 06 2022)

#### ⚠️ Pushed to `master`

- updated export statement for Card component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.78 (Mon Jun 06 2022)

#### ⚠️ Pushed to `master`

- removed unused prop for now ([@yondav](https://github.com/yondav))
- card component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.77 (Mon Jun 06 2022)

#### ⚠️ Pushed to `master`

- form invalid prop ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.76 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- svgProp typ ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.75 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- modal scroll handling ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.74 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- svgProp typo ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.73 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- stroke color on add and subtract icons ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.72 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- add and subtract icons ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.71 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- scroll behavior modal ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.70 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- inner modal container height ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.69 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- modal height ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.68 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- navLink styles ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.67 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- navlink styles ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.66 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- modal width ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.65 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- max width on modal component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.64 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- modal scrolling, overlay background darkened ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.63 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- modal export ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.62 (Sun Jun 05 2022)

#### ⚠️ Pushed to `master`

- close icon, modal component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.61 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- cursor on checkbox ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.60 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- form group spacing ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.59 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- form component touchup ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.58 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- form export ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.57 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- form export ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.56 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- form elements ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.55 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- input and text area ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.54 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- stickynav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.53 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- footer spacing ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.52 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- overlay transition ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.51 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- sidenav close ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.50 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- nav transition ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.49 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- removed fixed position from non sticky nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.48 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- sticky nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.47 (Sat Jun 04 2022)

#### ⚠️ Pushed to `master`

- no position needed for footer ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.46 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- fixed position footer ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.45 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- footer background color ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.44 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- export of Footer component and height calculation for side nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.43 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- no props needed for footer ([@yondav](https://github.com/yondav))
- footer component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.42 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- nav alignment ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.41 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- cyan order ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.40 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- updated nav bar and greens in the pallet ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.39 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- updated color pallet and nav component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.38 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- transitions ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.37 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- removed export of image ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.36 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- aws image ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.35 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- image export ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.34 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- repaired export of Nav ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.33 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- beginning of nav component, testing image ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.32 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- you tube icon and social button ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.31 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- magic eden icon width ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.30 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- Icon additions, Button fine tuning ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.29 (Fri Jun 03 2022)

#### ⚠️ Pushed to `master`

- styled components to dev dependencies and globalstyles generic export ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.28 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- exclude node modules ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.27 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react is not defined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.26 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react is undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.25 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react is undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.24 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- defaulting module exports in commonjs ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.23 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- commonjs config to include globals ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.22 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging publish ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.21 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging publish ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.20 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging publish ([@yondav](https://github.com/yondav))
- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.19 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.18 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.17 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.16 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.15 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- debugging react undefined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.14 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- debugging react not defined ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.13 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- publish test ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.12 (Thu Jun 02 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- more explicit types for build ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.11 (Wed Jun 01 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- not minified ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.10 (Wed Jun 01 2022)

#### ⚠️ Pushed to `master`

- npmignore ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.9 (Tue May 31 2022)

#### ⚠️ Pushed to `master`

- module type tsconfig ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.8 (Tue May 31 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- ts rollup config ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.7 (Tue May 31 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- importing react into components for bundle ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.6 (Tue May 31 2022)

#### ⚠️ Pushed to `master`

- modified icon rendering for social button component ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.5 (Tue May 31 2022)

#### 🐛 Bug Fix

- rollup configured, icon component consolidated [#1](https://github.com/new-technologies-group/story-book/pull/1) ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.4 (Tue May 31 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- module main js modified in package.json. Will work to configure whole build with rollup ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.3 (Sat May 28 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- added solana icon to icon list ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.2 (Thu May 26 2022)

#### ⚠️ Pushed to `master`

- Merge branch 'master' of github.com:new-technologies-group/story-book ([@yondav](https://github.com/yondav))
- seems to be publishing entire repo rather than only dist dir ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.1.1 (Thu May 26 2022)

#### ⚠️ Pushed to `master`

- setting up auto publishing ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))

---

# v0.0.1 (Thu May 26 2022)

#### ⚠️ Pushed to `master`

- experimenting with component distrobution across multiple repos ([@yondav](https://github.com/yondav))

#### Authors: 1

- Yoni David ([@yondav](https://github.com/yondav))
